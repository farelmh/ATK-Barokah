...
public class LapPenjualan extends javax.swing.JFrame {
    ...
    class Penjualan {
        ...
        public Penjualan(String id, String namaKasir, Date tanggal, double total) {
            this.id = id;
    ...
    class DetailPenjualan {
        ...
        public DetailPenjualan(String idPenjualan, String namaBarang, int jumlah, double subtotal) {
    ...
    private void setupTableAppearance() {
        ...
        tbl_penjualan.getColumnModel().getColumn(4).setCellRenderer(new ExpandButtonRenderer());
        tbl_penjualan.getColumnModel().getColumn(4).setCellEditor(new ExpandButtonEditor());

        // Set lebar kolom
        TableColumnModel columnModel = tbl_penjualan.getColumnModel();
        columnModel.getColumn(0).setPreferredWidth(100);  // ID Penjualan
        columnModel.getColumn(1).setPreferredWidth(120);  // Tanggal
        columnModel.getColumn(2).setPreferredWidth(140);  // Nama Kasir
        columnModel.getColumn(3).setPreferredWidth(120);  // Total Penjualan
        columnModel.getColumn(4).setPreferredWidth(50);   // Tombol Expand 
    ...
    private int getRowIndex(int penjualanIndex, boolean isExpanded) {
        ...
        for (int i = 0; i < penjualanIndex; i++) {
            if (expandedRows[i]) {
                index += detailPenjualanMap.get(daftarPenjualan.get(i).getId()).size();
            }
        }
        return index;
    }
    ...
    private boolean isDetailRow(int row) {
        ...
        for (int i = 0; i < daftarPenjualan.size(); i++) {
            ...
            if (expandedRows[i]) {
                List<DetailPenjualan> details = detailPenjualanMap.get(daftarPenjualan.get(i).getId());
    ...
    private int getPenjualanIndex(int row) {
        ...
        for (int i = 0; i < daftarPenjualan.size(); i++) {
            ...
            if (expandedRows[i]) {
                currentRow += detailPenjualanMap.get(daftarPenjualan.get(i).getId()).size();
            }
        }
        return -1;
    }
    ...
    private void cariDataTanggal(String mulai, String akhir) {
        daftarPenjualan.clear();
        detailPenjualanMap.clear();

        daftarPenjualan = new ArrayList<>();
        detailPenjualanMap = new HashMap<>();

        try {
            PreparedStatement statPenjualan = k.getCon().prepareStatement("CALL getDataPenjualan(?, ?)");
            statPenjualan.setString(1, mulai);
            statPenjualan.setString(2, akhir);

            ResultSet rsPenjualan = statPenjualan.executeQuery();
            while (rsPenjualan.next()) {
                String id = rsPenjualan.getString("id_jual");
                Date tanggal = rsPenjualan.getDate("tanggal_jual");
                String kasir = rsPenjualan.getString("nama_karyawan");
                double total = rsPenjualan.getDouble("total");

                Penjualan penjualan = new Penjualan(id, kasir, tanggal, total);
                daftarPenjualan.add(penjualan);

                PreparedStatement statDetail = k.getCon().prepareStatement("CALL getDetailJual(?)");
                statDetail.setString(1, id);
                ResultSet rsDetail = statDetail.executeQuery();
                ...
                while (rsDetail.next()) {
                    ...
                    listDetail.add(new DetailPenjualan(id, namaBarang, jumlah, subtotal));
                }
                detailPenjualanMap.put(id, listDetail);
            ...
            expandedRows = new boolean[daftarPenjualan.size()];
            Arrays.fill(expandedRows, false);

            tableModel.setData(daftarPenjualan, detailPenjualanMap);
            ((ExpandableTableModel) tbl_penjualan.getModel()).fireTableDataChanged();
    ...
    private void isiTabel() {
        ...
        try {
            this.statPenjualan = k.getCon().prepareStatement("SELECT * FROM v_lap_penjualan");
            this.rsPenjualan = this.statPenjualan.executeQuery();
            while (rsPenjualan.next()) {
                String id = rsPenjualan.getString("id_jual");
                Date tanggal = rsPenjualan.getDate("tanggal_jual");
                String kasir = rsPenjualan.getString("nama_karyawan");
                ...
                this.statDetail = k.getCon().prepareStatement("CALL getDetailJual(?)");
                statDetail.setString(1, id);
    ...
    class ExpandableTableModel extends AbstractTableModel {
        ...
        private String[] namaKolom = {"ID Penjualan", "Tanggal", "Kasir", "Total", "Detail"};
        private SimpleDateFormat dateFormat = new SimpleDateFormat("dd-MM-yyyy");
        private List<Penjualan> data = new ArrayList<>();

        @Override
        public int getRowCount() {
            int total = daftarPenjualan.size();
            for (int i = 0; i < expandedRows.length; i++) {
                if (expandedRows[i]) {
                    total += detailPenjualanMap.get(daftarPenjualan.get(i).getId()).size();
                }
            }
            return total;
        }

        public void setData(List<Penjualan> newJual, Map<String, List<DetailPenjualan>> newDetail) {
            daftarPenjualan = newJual;
            detailPenjualanMap = newDetail;
        ...
        public boolean isCellEditable(int rowIndex, int columnIndex) {
            return columnIndex == 4 && !isDetailRow(rowIndex);
        }
        ...
        public Object getValueAt(int rowIndex, int columnIndex) {
            ...
            for (int i = 0; i < daftarPenjualan.size(); i++) {
                if (currentRow == rowIndex) {
                    ...
                    Penjualan p = daftarPenjualan.get(i);
                    switch (columnIndex) {
                        case 0:
                            return p.getId();
                        case 1:
                ...
                if (expandedRows[i]) {
                    ...
                    for (int j = 0; j < details.size(); j++) {
                        if (currentRow == rowIndex) {
                            ...
                            DetailPenjualan d = details.get(j);
                            switch (columnIndex) {
                                case 0:
    ...
    private void exportExcel() {
        ...
        try (Workbook workbook = new XSSFWorkbook()) {
            ...
            Cell cellLabel = totalRow.createCell(2); // 
            cellLabel.setCellValue("Total Penjualan :");
            ...
            Cell cellLabel1 = totalRow1.createCell(3); // kolom "Kasir"
            cellLabel1.setCellValue("Total Barang Terjual :");
            cellLabel1.setCellStyle(headerStyle); // atau style tebal

            Cell cellTotal1 = totalRow1.createCell(4); // kolom "Total"
            String kolomTotal1 = "E"; // misal kolom D untuk total
            ...
            Cell cellLabel2 = totalRow2.createCell(5); // kolom "Kasir"
            cellLabel2.setCellValue("Total Penjualan :");
            cellLabel2.setCellStyle(headerStyle); // atau style tebal

            Cell cellTotal2 = totalRow2.createCell(6); // kolom "Total"
            String kolomTotal2 = "G"; // misal kolom D untuk total

            String bersih2 = totalJual.getText();
